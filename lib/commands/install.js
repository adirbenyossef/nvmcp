...
async function installCommand(args, options) {
  ...
  try {
    ...
    if (packageSpec.type === 'npm') {
      ...
      await installNpmPackage(packageInfo, options);
    ...
    await createDefaultConfig(packageSpec.name, packageInfo, options);
...
async function installNpmPackage(packageInfo, options) {
  const serverDir = path.join(SERVERS_DIR, packageInfo.name);
...
async function createDefaultConfig(serverName, packageInfo, options) {
  const config = {
    name: serverName,
