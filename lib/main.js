...
const COMMAND_REGISTRY = {
  ...
  quickstart: {
    handler: quickstartCommand,
    description: 'Quick setup with popular MCP presets',
    category: 'setup',
    aliases: ['quick']
  },
  presets: {
    handler: presetsCommand,
    description: 'Show available MCP presets',
    category: 'setup',
    aliases: []
  },
...
function organizeCommandsByCategory() {
  ...
  for (const [name, config] of Object.entries(COMMAND_REGISTRY)) {
    ...
    case 'setup':
      categories.setupCommands.push(commandInfo);
      break;
    case 'tag':
...
function showHelp(specificCommand = null) {
  ...
  const { setupCommands, tagCommands, mcpCommands, processCommands } = organizeCommandsByCategory();
...
${formatCommandList(setupCommands)}
...
${colors.bold('EXAMPLES:')}
  ${colors.dim('nvmcp init')}                              First-time setup wizard
  ${colors.dim('nvmcp quickstart whatsapp-dev')}           Quick WhatsApp setup
  ${colors.dim('nvmcp presets')}                           Show all available presets
  ${colors.dim('nvmcp create development')}
  ${colors.dim('nvmcp add whatsapp')}                      Add WhatsApp MCP preset
  ${colors.dim('nvmcp add github:lharries/whatsapp-mcp')} Add from GitHub
  ${colors.dim('nvmcp use development --start --claude')} Switch and export
  ${colors.dim('nvmcp ps')}`);
}
