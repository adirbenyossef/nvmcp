...
let _colorSupportCached = null;

function isColorSupported() {
  if (_colorSupportCached !== null) {
    return _colorSupportCached;
  }

  const env = process.env;

  if (env.FORCE_COLOR) {
    _colorSupportCached = true;
    return true;
  }

  if (env.NO_COLOR || env.NODE_DISABLE_COLORS) {
    _colorSupportCached = false;
    return false;
  }

  _colorSupportCached = !!(process.stdout && process.stdout.isTTY);
  return _colorSupportCached;
}
...
function formatTable(headers, rows, _options = {}) {
