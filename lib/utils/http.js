    ...
    try {
      ...
      const req = client.request(requestOptions, (res) => {
        ...
        res.on('end', () => {
          ...
          if (res.statusCode >= NETWORK.HTTP.STATUS_CODES.OK && res.statusCode < NETWORK.HTTP.STATUS_CODES.REDIRECT_START) {
            resolve(response);
          } else {
...
function post(url, data, options = {}) {
  const headers = { 'Content-Type': NETWORK.HTTP.HEADERS.CONTENT_TYPE, ...options.headers };
...
function downloadFile(url, destination, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(requestOptions, (res) => {
      if (res.statusCode >= NETWORK.HTTP.STATUS_CODES.REDIRECT_START && res.statusCode < NETWORK.HTTP.STATUS_CODES.REDIRECT_END && res.headers.location) {
      ...
      if (res.statusCode !== NETWORK.HTTP.STATUS_CODES.OK) {
        reject(new Error(`HTTP ${res.statusCode}: ${res.statusMessage}`));
        return;
      }
