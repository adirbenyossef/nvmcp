...
const { getNpmPackageInfo, getGithubRepoInfo } = require('./http');
...
class ProcessManager {
  ...
  async resolveNpmCommand(sourceInfo) {
    ...
    try {
      // Get package info to validate it exists and get metadata
      ...
      // Fallback to npx if package info fetch fails
      console.warn(`Could not fetch package info for ${packageName}, using npx directly`);
      return {
        cmd: 'npx',
        args: [packageName]
      };
  ...
  async resolveGithubCommand(sourceInfo) {
    ...
    try {
      // Get repository info to validate it exists
      await getGithubRepoInfo(sourceInfo.fullRepo);
    } catch (error) {
      console.warn(`Could not fetch repo info for ${sourceInfo.fullRepo}, proceeding with clone`);
    }
